...
data_pre <- data_sorted[c("date", "deaths_covid", "hospital_onset_covid")]
...
X_train <- Ts_mbd[, -1:-2] #the lags (death and other variables) and data from the other variables
...
X_test <- Ts_mbd[nrow(Ts_mbd), c((ncol(Tsib)):(2*(lag_order+1)))] #this creates lags to use to predict the actual value of Y
